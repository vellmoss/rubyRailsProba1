...
describe "User pages" do
  ...
  describe "edit" do
    ...
    describe "page" do
      it { should have_content("Изменение профиля") }
      it { should have_title("Edit user") }
      it { should have_link('поменять', href: 'http://gravatar.com/emails') }
    end

    describe "with invalid information" do
      before { click_button "Запомнить" }
    ...
    describe "with valid information" do
      let(:new_name)  { "vell2" }
      let(:new_email) { "vellmoss@gmail.com" }
      before do
        fill_in "Name",             with: new_name
        fill_in "Email",            with: new_email
        fill_in "Password",         with: user.password
        fill_in "Повторение пароля:", with: user.password
        click_button "Запомнить"
      end
